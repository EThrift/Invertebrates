#created on 22/08/24
# By Emily Thrift
# script for a glm to assess the levels of polymer levels in trophic levels of different invertebrate samples tested 
...
df_polymer_trophic <- read.csv("Polymer.csv")


# Remove spaces from column names
colnames(df_polymer_trophic) <- make.names(colnames(df_polymer_trophic))
...
df_pca$Trophic.level <- as.factor(df_pca$Trophic.level)
...
pca_coords$label_x <- pca_coords$Dim.1 * 3.2  # Position label at 90% of the arrow length on x-axis
pca_coords$label_y <- pca_coords$Dim.2 * 3.2  # Position label at 90% of the arrow length on y-axis

# Plot with labels positioned near the arrowheads
fviz_pca_biplot(
  pca_result,
  label = "none",                      # Do not show default variable labels
  habillage = df_pca$Trophic.level,    # Color points by Trophic level
  addEllipses = TRUE,                  # Add concentration ellipses
  palette = "Set1"                     # Set color palette
) + 
  geom_point(aes(color = df_pca$Trophic.level), size = 4, shape = 16) +  # Round points only
